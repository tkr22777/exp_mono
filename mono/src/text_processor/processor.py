...
def generate_llm_response(prompt: str, session_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        print(
            f"[LLM Request] Session: {session_id}, Messages: {json.dumps(messages, indent=2)}"
        )
        ...
        if session_id and response:
            ...
            print(
                f"[Session State] Session: {session_id}, History length: {len(state['history'])}"
            )
